...
namespace FlashCards.obitom67
{
    public class FlashCard
    {
        ...
        public static void CreateFlashCard(Stack stack)
        {
            ...
            {
                connection.Open();
                string count = $"SELECT COUNT(*) FROM FlashCards.dbo.Flashcard WHERE StackId = {stack.StackId}";
                var flashcardCount = connection.ExecuteScalar(count);
                string sql = $"INSERT INTO FlashCards.dbo.Flashcard(FrontText,BackText,FlashcardId,StackId) VALUES ('{flashCard.FrontText}','{flashCard.BackText}',{(int)flashcardCount + 1},{stack.StackId})";
        ...
        public static void UpdateFlashCards(Stack stack)
        {
            ...
            using(var connection = new SqlConnection(connectionString))
            {
                string sql = $"SELECT * FROM FlashCards.dbo.Flashcard WHERE StackId = {stack.StackId}";
                ...
                string updateSql = $"UPDATE FlashCards.dbo.Flashcard SET FrontText = '{flashcardChange.FrontText}',BackText = '{flashcardChange.BackText}' WHERE FlashcardId = {updateId}";
        ...
        public static void DeleteFlashCard(Stack stack)
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                string sql = $"DELETE FROM FlashCards.dbo.Flashcard WHERE FlashcardId = {deleteId}";
                connection.Execute(sql);
                string sqlCount = $"SELECT * FROM FlashCards.dbo.Flashcard WHERE StackId = {stack.StackId}";
                var flashcards = connection.Query(sqlCount);
                int count = 1;
                foreach (var flashcard in flashcards)
                {
                    string updateId = $"UPDATE FlashCards.dbo.Flashcard SET FlashcardId = {count} WHERE FlashcardId = {flashcard.FlashcardId}";
