...
public class StudyController
{
    ...
    public List<StudyDto> GetAllStudySessions(int stack_id) =>
        ExecuteQuery("SELECT studyDate, score, flashcard_amount FROM Study WHERE stack_id = @stack_id",
            new { stack_id });

    public void InsertStudySession(string date, int score, int flashcardAmount, int stack_id)
    {
        var query =
            @"INSERT INTO Study(studyDate, score, flashcard_amount, stack_id) 
                        VALUES (@Date, @Score, @FlashcardAmount, @StackId)";
        var parameters = new { Date = date, Score = score, FlashcardAmount = flashcardAmount, StackId = stack_id };
        ExecuteNonQuery(query, parameters);
    }

    public void DeleteAll(int stack_id) =>
        ExecuteNonQuery("DELETE FROM Study WHERE stack_id = @Stack_id", new { Stack_Id = stack_id });
