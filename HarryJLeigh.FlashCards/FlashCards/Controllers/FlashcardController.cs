...
public class FlashcardController
{
    ...
    public FlashcardController(DatabaseService dbService) => DbService = dbService;
    
    public List<FlashCardDto> GetAllFlashcards(int stack_id) => ExecuteQuery("Select flashCardId, front, back FROM Flashcards WHERE stack_id = @Stack_id", new {Stack_id = stack_id });
    public List<FlashCardDto> GetAmountOfFlashcards(string limit, int stack_id ) => ExecuteQuery($"SELECT TOP {limit} flashCardId, front, back FROM Flashcards WHERE stack_id = @Stack_id", new {Stack_id = stack_id });
    ...
    public void DeleteAllFlashcards(int stack_id) => ExecuteNonQuery($"DELETE FROM Flashcards WHERE stack_id = {stack_id}");
