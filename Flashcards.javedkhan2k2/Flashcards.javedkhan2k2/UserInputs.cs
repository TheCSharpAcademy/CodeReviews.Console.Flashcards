...
internal static class UserInputs
{
    ...
    {
        string? front = GetStringInput("Please enter the [bold blue]front[/] of the Flash Card");
        ...
        while (dbContext.GetFlashCardByStackIdAndFront(stack.Id, front) != null)
        {
            ...
            front = GetStringInput("Please enter the [bold blue]front[/] of the Flash Card");
            if (front == null) return (null, null);
        }
        string? back = GetStringInput("Please enter the [bold blue]back[/] of the Flash Card");
    ...
    {
        string? input = AnsiConsole.Ask<string>($"{message} {BackMessage}");
        if (Validations.IsStringEqualsZero(input)) return null;

        while (string.IsNullOrWhiteSpace(input))
        {
            input = AnsiConsole.Ask<string>($"{InvalidInputMessage} {message} {BackMessage}");
            if (Validations.IsStringEqualsZero(input)) return null;
        }
